...
const AddParticipantForm = () => {
  ...
  useEffect(() => {
    ...
    if (token && token.startsWith("team-token-")) {
      setTeamId(token.split("-")[2]);
    } else {
      toast.error("You must be logged in as a team to add participants.");
      router.push("/login");
    }
      ...
      <Card className="w-full max-w-2xl mx-auto mt-6 shadow-sm border">
        <form onSubmit={handleSubmit}>
          <CardHeader className="bg-muted/30 pb-6 border-b">
            <CardTitle className="text-2xl font-bold">Add Participant</CardTitle>
            <CardDescription>
              Register a student for your team and assign their competitions.
            </CardDescription>
          </CardHeader>
